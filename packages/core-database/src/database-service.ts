...
import { Blockchain, Database, EventEmitter, Logger } from "@phantomchain/core-interfaces";
import { roundCalculator } from "@phantomchain/core-utils";
import { Bignum, constants, crypto as phantomCrypto, models } from "@phantomchain/crypto";
...
export class DatabaseService implements Database.IDatabaseService {
    ...
    public async verifyTransaction(transaction: models.Transaction) {
        const senderId = phantomCrypto.getAddress(transaction.data.senderPublicKey, this.config.get("network.pubKeyHash"));
