...
import { phantomToSatoshi, generateTransactions, logger, parseFee, satoshiToPhantom } from "../utils";
...
export class MultiSignatureCommand extends BaseCommand {
    ...
    public generateTransactions(wallets, approvalWallets = [], publicKeys = [], min = 2, log = true) {
        ...
        wallets.forEach((wallet, i) => {
            ...
            if (log) {
                logger.info(`${i} ==> ${transaction.id}, ${wallet.address} (fee: ${satoshiToPhantom(transaction.fee)})`);
            }
        });
    ...
    public async testSendWithSignatures(wallets, approvalWallets = []) {
        ...
        const transactions = generateTransactions(phantomToSatoshi(2), wallets, approvalWallets, {
            config: this.config,
            ...this.options,
        });
    ...
    public async testSendWithMinSignatures(wallets, approvalWallets = [], min = 2) {
        ...
        const transactions = generateTransactions(phantomToSatoshi(2), wallets, take(approvalWallets, min), {
            config: this.config,
            ...this.options,
        });
    ...
    public async testSendWithBelowMinSignatures(wallets, approvalWallets = [], min = 2) {
        ...
        const transactions = generateTransactions(phantomToSatoshi(2), wallets, take(approvalWallets, max), {
            config: this.config,
            ...this.options,
        });
    ...
    public async testSendWithoutSignatures(wallets) {
        ...
        const transactions = generateTransactions(phantomToSatoshi(2), wallets, [], {
            config: this.config,
            ...this.options,
        });
