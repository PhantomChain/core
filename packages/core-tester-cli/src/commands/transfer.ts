...
import { phantomToSatoshi, generateTransactions, logger, satoshiToPhantom } from "../utils";
...
export class TransferCommand extends BaseCommand {
    ...
    public async run(): Promise<void> {
        ...
        if (!this.options.skipValidation) {
            logger.info(`Sender starting balance: ${satoshiToPhantom(walletBalance)}`);
        }

        let totalDeductions = Bignum.ZERO;
        const transactionAmount = phantomToSatoshi(this.options.amount || 2);
        ...
        if (!this.options.skipValidation) {
            logger.info(`Sender expected ending balance: ${satoshiToPhantom(expectedSenderBalance)}`);
        }
    ...
    public async sendTransactionsWithResults(runOptions, isSubsequentRun) {
        ...
        if (runOptions.primaryAddress && runOptions.expectedSenderBalance) {
            ...
            if (!walletBalance.isEqualTo(runOptions.expectedSenderBalance)) {
                successfulTest = false;
                logger.error(
                    `Sender balance incorrect: '${satoshiToPhantom(walletBalance)}' but should be '${satoshiToPhantom(
        ...
        for (const wallet of runOptions.wallets) {
            ...
            if (!balance.isEqualTo(runOptions.transactionAmount)) {
                successfulTest = false;
                logger.error(
                    `Incorrect destination balance for ${wallet.address}. Should be '${satoshiToPhantom(
                        runOptions.transactionAmount,
                    )}' but is '${satoshiToPhantom(balance)}'`,
                );
            }
        }
    ...
    public async testVendorField(wallets) {
        ...
        const transactions = this.generateTransactions(phantomToSatoshi(2), wallets, null, null, "Testing VendorField");
    ...
    public async testEmptyVendorField(wallets) {
        ...
        const transactions = this.generateTransactions(phantomToSatoshi(2), wallets, null, null, null);
