...
export function generateTransactions(
    ...
    wallets.forEach((wallet, i) => {
        ...
        if (options.log) {
            logger.info(
                `${i} ==> ${transaction.id}, ${transaction.recipientId} (fee: ${this.satoshiToPhantom(transaction.fee)})`,
            );
        }
    });
...
/**
 * Convert PHANTOM to Satoshi.
 * @param  {Number} phantom
 * @return {Bignum}
 */
export function phantomToSatoshi(phantom) {
    return bignumify(phantom * 1e8);
}

/**
 * Convert Satoshi to PHANTOM.
 * @param  {Bignum} satoshi
 * @return {String}
 */
export function satoshiToPhantom(satoshi) {
