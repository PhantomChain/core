...
const main = async () => {
    let {
        ...
        type: 'text',
        name: 'corePath',
        initial: expandHomeDir('~/phantom-core'),
    ...
    // Paths
    const corePaths = envPaths('phantom', {
        suffix: 'core'
    });
    ...
    fs.writeFileSync(`${paths.config.new}/.env`, envCurrent.replace(new RegExp('PHANTOM_', 'g'), 'CORE_'));
